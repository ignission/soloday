...
**Summary:** SQLiteデータベース基盤の実装。better-sqlite3を使用したシングルトンパターンによる接続管理、マイグレーション自動実行機能、Result/Option型による型安全なエラーハンドリングを実装。データベースは ~/.soloday/db.sqlite に作成され、settingsテーブルとmigrationsテーブルが初期マイグレーションで作成される。
